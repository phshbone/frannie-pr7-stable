...
- 50 executable state/structure assertions pass: the preserved PR7 sitter/state behavior, isolated cache cleanup, viewport-owned scrolling, body-anchored absolute navigation, the original three-second splash, multiple current medications/foods, append-only audit union, secure invite and recovery URLs, official imagery/icons/manifest, restored training library, iPhone modal cleanup, and complete PWA asset alignment.
...
- The isolated cache and query versions (`app 33`, styles 35, shared core 17, shared UI 18, training interface 2) agree.
